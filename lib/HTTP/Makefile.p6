...
# recompiles.
my $parrot_dir = %*VM<config><prefix>;
say "PARROT_DIR = $parrot_dir (r{%*VM<config><revision>})";

# RAKUDO_DIR may be in or out of the parrot tree. Needed for Test.pm.
my $rakudo_dir = $perl6.subst( $parrot_dir~'/parrot', '' ); # remove possible parrot
...
$maketext .= subst( '<PARROT_DIR>', $parrot_dir );
